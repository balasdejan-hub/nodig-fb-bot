...
# Deduplication — pamti zadnjih N mid-ova da se izbjegnu dupli odgovori
SEEN_MIDS = set()
MAX_SEEN_MIDS = 500
...
def send_action(recipient_id, action="typing_on"):
    """Pošalji sender action (typing_on / typing_off / mark_seen)."""
    payload = {
...
def detect_language(text):
    ...
    # --- Keyword hints po jeziku (prioritet redom) ---

    de_keywords = [
        "preis", "händler", "haendler", "kaufen", "guten tag", "hallo",
        "wo kaufen", "wo kann", "deutsch", "deutschland", "österreich", "schweiz"
    ]
    es_keywords = [
        "precio", "comprar", "distribuidor", "catálogo", "hola", "dónde",
        "donde", "donde comprar", "españa", "español"
    ]
    it_keywords = [
        "prezzo", "comprare", "distributore", "ciao", "dove",
        "dove acquistare", "italia", "italiano"
    ]
    fr_keywords = [
        "prix", "acheter", "distributeur", "catalogue", "bonjour",
        "où", "ou acheter", "france", "français", "belgi"
    ]
    sl_keywords = [
        "živjo", "zivjo", "slovenija", "kje kupiti", "kje", "slovensko"
    ]
    hr_keywords = [
        "cijena", "gdje", "pozdrav", "bok", "hrvatska", "kupiti hr", "dobro jutro hr"
    ]
    sr_keywords = [
        "cena", "gde", "distributer", "beograd", "srbija", "zdravo sr"
    ]
    bs_keywords = [
        "cijenu", "gdje kupiti", "ovdje", "možete", "bosna", "sarajevo"
    ]
    en_keywords = [
        "price", "catalog", "catalogue", "dealer", "buy", "hello", "hi",
        "hey", "where", "how much", "distributor", "product", "brochure",
        "cost", "quote", "purchase", "reseller", "english", "info"
    ]

    if any(x in t for x in de_keywords):
        return "de"
    if any(x in t for x in es_keywords):
        return "es"
    if any(x in t for x in it_keywords):
        return "it"
    if any(x in t for x in fr_keywords):
        return "fr"
    if any(x in t for x in sl_keywords):
        return "sl"
    if any(x in t for x in hr_keywords):
        return "hr"
    if any(x in t for x in sr_keywords):
        return "sr"
    if any(x in t for x in bs_keywords):
        return "bs"
    if any(x in t for x in en_keywords):
        return "en"

    # --- langdetect samo za dulje tekstove (4+ riječi) ---
    if len(t.split()) >= 4:
        try:
            detected = detect(t)
    ...
    # --- Sve ostalo (arapski, hebrejski, kineski...) → engleski ---
    return DEFAULT_LANG
...
def handle_message(sender_id, user_text):
    ...
    # Typing indicator — prirodniji feel
...
def handle_postback(sender_id, payload):
    ...
    if payload == "GET_STARTED":
        return send_text_message(
            sender_id,
            get_localized_text("en", "greeting")
        )

    return send_text_message(
        sender_id,
        get_localized_text("en", "fallback")
    )
...
def webhook():
    ...
    for entry in data.get("entry", []):
        ...
        for messaging_event in entry.get("messaging", []):
            ...
            if not sender_id:
                print("[WEBHOOK] No sender_id, skipping.")
                continue
            ...
            if "message" in messaging_event:
                ...
                # Ignoriraj echo poruke (page je sama poslala)
                if message_obj.get("is_echo"):
                    print("[WEBHOOK] Echo message — ignoring.")
                    continue

                # Deduplication po mid
                ...
                if mid:
                    ...
                    if len(SEEN_MIDS) > MAX_SEEN_MIDS:
                        # Obrisi najstarijih — jednostavan cleanup
                        oldest = next(iter(SEEN_MIDS))
                        SEEN_MIDS.discard(oldest)

                text = (message_obj.get("text") or "").strip()
                attachments = message_obj.get("attachments", [])

                if text:
                    print(f"[WEBHOOK] Text message: {text!r}")
                    handle_message(sender_id, text)
                    continue

                if attachments:
                    print("[WEBHOOK] Attachment received.")
                    handle_attachment(sender_id, message_obj)
                    continue

                print("[WEBHOOK] Message without text or attachments.")
                continue
            ...
            if "pass_thread_control" in messaging_event:
                print("[WEBHOOK] pass_thread_control")
                continue
            if "take_thread_control" in messaging_event:
                print("[WEBHOOK] take_thread_control")
                continue
            if "request_thread_control" in messaging_event:
                print("[WEBHOOK] request_thread_control")
                continue

            # --- Referral ---
            if "referral" in messaging_event:
                print(f"[WEBHOOK] referral: {json.dumps(messaging_event.get('referral', {}), ensure_ascii=False)}")
                continue

            print("[WEBHOOK] Unhandled event type.")

        # Standby channel eventi
